import {
  ...
  type Column = {
    ...
    cell?: (props: { row: { original: HargaObat; index: number }; meta: { onEdit: (data: HargaObat) => void; onDelete: (data: HargaObat) => void; currentPage: number; itemsPerPage: number} }) => React.ReactNode
  }
            ...
            <TableBody>
              {loading ? (
                ...
                data.map((row, rowIndex) => (
                  <TableRow key={`${row.id_klinik}-${row.id_obat}-${rowIndex}`}>
                    {columns.map((column) => (
