...
const PortalLayout: React.FC<PortalLayoutProps> = ({ children, role, dark = false }) => {
  ...
  return (
    <div className={`min-h-screen flex flex-col ${dark ? 'bg-[#030712] text-white' : 'bg-slate-50/20'} relative font-sans transition-colors duration-300 overflow-hidden`}>
      {/* Top Header */}
      <header className="bg-gradient-to-r from-blue-800 via-indigo-700 to-purple-800 px-6 py-3.5 flex items-center justify-between border-b border-indigo-900/40 shadow-lg relative z-50 transition-all duration-300">
        {/* Left: Logo & Nav */}
      ...
      {role !== 'doctor' && isMobileMenuOpen && (
        <div className="md:hidden bg-blue-700 border-b border-blue-800 shadow-lg px-6 py-4 space-y-3 absolute w-full z-40 transition-all">
          <Link
