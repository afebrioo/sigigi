import {
  ...
  type Column = {
    ...
    cell?: (props: { row: { original: HargaTindakan; index: number }; meta: { onEdit: (data: HargaTindakan) => void; onDelete: (data: HargaTindakan) => void; currentPage: number; itemsPerPage: number} }) => React.ReactNode
  }
            ...
            <TableBody>
              {loading ? (
                ...
                data.map((row, rowIndex) => (
                  <TableRow key={`${row.id_klinik}-${row.id_master_tindakan}-${rowIndex}`}>
                    {columns.map((column) => (
